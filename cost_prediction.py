...
# loading the data
data = pd.read_excel(r"E:\DS project\preparation/pro_71.xlsx")
data.describe()
data.info()
...
sns.relplot(x='Competitor_Price' , y='Duration_of_coaching_in_Hours', data=data)
sns.relplot(x='Competitor_Price' , y='Maintenance_cost', data=data)
sns.relplot(x='Competitor_Price' , y='Debentures', data=data)
...
### Normalization
df_new.columns
df_new.drop(['Competitor_Names'], axis = 1, inplace = True)
df_new.drop(['Certificate_issued_or_not'], axis = 1, inplace = True)
a1 = df_new.describe()
...
import seaborn as sns
sns.pairplot(df_norm.iloc[9:, 14:])
sns.pairplot(df_norm, hue = "Competitor_")
# Correlation matrix 
df_norm.corr()
df_norm.summary
...
# index 7,54 is showing high influence so we can exclude that entire row

ml1 = smf.ols('Competitor_Price ~ Maintenance_cost + Marketing_cost + Debentures + Salary_of_the_trainer + Profit_Margin', data = df_norm).fit() # regression model
ml1.summary()
df_norm = df_norm.drop(df_norm.index[[54]])
sm.graphics.influence_plot(ml1)
